...
template <class T> class generator {
  struct promise {
    ...
    using reference_type = std::conditional_t<std::is_pointer_v<value_type>,
                                              value_type, value_type &>;
    using pointer_type = std::conditional_t<std::is_pointer_v<value_type>,
                                            value_type, value_type *>;

    // void *operator new(std::size_t n) noexcept {
    //   if (void *mem = ::operator new(n))
    //     return mem;
    //   return nullptr;
    // }

    // void operator delete(void *n) noexcept { ::operator delete(n); }
...
public:
  ...
  public:
    ...
    reference_type operator*() const noexcept(
        noexcept(std::is_nothrow_copy_constructible_v<reference_type>)) {
      if constexpr (std::is_pointer_v<value_type>)
